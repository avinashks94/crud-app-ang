...
export class LanguageDetectorComponent {
  ...
  setRepoType(type: RepoType) {
    ...
    // Clear results
    this.repoMeta.set(null);
    this.result.set(null);
    this.auditFindings = [];
    this.treeTruncated = false;
    // Optional: move focus to URL box (guarded for SSR)
  ...
  async analyze() {
    ...
    try {
      ...
      // 2) Default branch + tree: isolate errors so they don't override a successful languages call
      try {
        ...
        // Soft-fail: keep language results, show informational note instead of hard error
        const status = treeErr?.status;
        if (status === 404) {
          this.infoMsg.set(
            'Could not fetch file tree for useless-files audit (404). Language results are shown.'
          ); // [2]
        } else if (status === 403) {
          this.infoMsg.set(
            'File tree fetch forbidden/rate limited; try later or adjust token scopes.'
          ); // [1]
        } else {
          this.infoMsg.set('Useless-files audit skipped due to a fetch error.'); // [2]
        }
      }

      if (this.repoType() === 'private') {
        this.infoMsg.set('Authenticated: private repository access confirmed.'); // [1]
      }
    } catch (e: any) {
      ...
      if (status === 401) {
        this.errorMsg.set(
          'Unauthorized (401). Check token value and scopes (Contents: Read or repo).'
        ); // [1]
      } else if (status === 403) {
        this.errorMsg.set(
          'Forbidden (403). Rate limit or insufficient token permissions.'
        ); // [1]
      } else if (status === 404) {
        // Only show this if languages request failed
        this.errorMsg.set(
          'Repository not found or no access (404). Verify owner/repo and token permissions.'
        ); // [2]
      } else {
        this.errorMsg.set(msg || 'Request failed'); // [2]
      }
    } finally {
      this.loading.set(false);
    }
  ...
  private parseRepoUrl(input: string): { owner: string; repo: string } | null {
    const trimmed = input.trim().replace(/\/+$/, ''); // drop trailing slash [2]
